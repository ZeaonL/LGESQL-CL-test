...
def add_argument_base(arg_parser):
    ...
    arg_parser.add_argument('--task', default='lgesql_large', help='task name')
    arg_parser.add_argument('--seed', default=100, type=int, help='Random seed')

    # TODO CL
    arg_parser.add_argument('--data_dir', default='data/Spider', type=str, help='data dir')
    # arg_parser.add_argument('--exp_path', default='CL_soft_seed_100_l2_0.1', type=str, help='exp dir')
    arg_parser.add_argument('--CL_mode', default='', type=str, help='is CL mode')
    arg_parser.add_argument('--full_data', action='store_true', help='is small data mode')
    arg_parser.add_argument('--CL_epoch_num', default=40, type=int, help='CL_epoch_num')
    arg_parser.add_argument('--soft_CL_matrix_decay', default=1, type=int, help='soft_CL_matrix_decay')
    
    arg_parser.add_argument('--device', type=int, default=0, help='Use which device: -1 -> cpu ; the index of gpu o.w.')
    # arg_parser.add_argument('--cuda_visible_devices', type=str, default='2')
    arg_parser.add_argument('--testing', action='store_true', help='training or evaluation mode')
    arg_parser.add_argument('--read_model_path', default='', type=str, help='read pretrained model path')
    #### Training Hyperparams ####
    # TODO
    arg_parser.add_argument('--batch_size', default=35, type=int, help='Batch size')
    ...
    arg_parser.add_argument('--warmup_ratio', type=float, default=0.0, help='warmup steps proportion')
    
    arg_parser.add_argument('--lr_schedule', default='linear', choices=['constant', 'linear', 'ratsql', 'cosine'], help='lr scheduler')
    arg_parser.add_argument('--eval_after_epoch', default=0, type=int, help='Start to evaluate after x epoch')
...
def add_argument_encoder(arg_parser):
    # Encoder Hyperparams
    arg_parser.add_argument('--model', choices=['rgatsql', 'lgesql'], default='rgatsql', help='which text2sql model to use')
    ...
    arg_parser.add_argument('--plm', type=str, default='bert-base-uncased', choices=['bert-base-uncased', 'bert-large-uncased', 'bert-large-uncased-whole-word-masking',
        'roberta-base', 'roberta-large', 'grappa_large_jnt', 'electra-base-discriminator', 'electra-large-discriminator', 'bert_cn', 'bert-base-uncased'], help='pretrained model name')
