...
class Example():
    ...
    def configuration(cls, plm=None, method='lgesql', table_path='data/tables.json', tables='data/tables.bin', db_dir='data/database'):
        ...
        if plm is None:
            ...
            # cls.word_vocab = Vocab(padding=True, unk=True, boundary=True, default=UNK,
            #     filepath='./pretrained_models/glove.42b.300d/vocab_SeSQL.txt', specials=SCHEMA_TYPES) # TODO
        else:
    ...
    @classmethod
    def load_dataset(cls, choice, debug=False, data_dir='data'):
        assert choice in ['train', 'dev', 'test']
        fp = os.path.join(data_dir, choice + '.' + cls.method + '.bin')
        datasets = pickle.load(open(fp, 'rb'))
        # datasets = datasets[:500]   # TODO
