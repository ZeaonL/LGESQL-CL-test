...
def hyperparam_path_text2sql(args):
    ...
    # TODO CL
    exp_path = 'CL_%s__' % (args.CL_mode)
    
    # encoder params
    exp_path += 'emb_%s' % (args.embed_size) if args.plm is None else 'plm_%s' % (args.plm)  # TODO
    # exp_path = 'emb_%s' % (args.embed_size) if args.plm is None else 'plm_%s' % (args.plm)
    exp_path += '__l2_%s' % (args.l2)   # TODO
    exp_path += '__wp_%s' % (args.warmup_ratio) # TODO
