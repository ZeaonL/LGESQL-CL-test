...
def parse_col(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    for alias in default_tables:
        ...
        if (tok in schema.schema[table]) or (tok.upper() in schema.schema[table]):  # TODO
...
if __name__ == '__main__':
    ...
    ALL_DB_SCHEMA = get_all_schema("/data4/zyliu/Program/LGESQL/data/SeSQL/tables.json")
    
    schema = ALL_DB_SCHEMA['企业股东']
    schema = Schema(schema)
    sql = "select 股东.姓名 from 股东 join 企业股东 on 企业股东.股东id = 股东.词条id where 企业股东.任职职位='董事长'"
    print(get_sql(schema, sql))
    
    import os, json
    data_dict = ['dev.json', 'test.json']
    data = []
    for data_path in data_dict:
        path = os.path.join("/data4/zyliu/Program/LGESQL/data/SeSQL/", data_path)
        with open(path) as fp:
            data.extend(json.load(fp))
    for content in data:
        schema = ALL_DB_SCHEMA[content['db_id']]
        schema = Schema(schema)
        sql = content['query']
        print(sql)
        get_sql(schema, sql)
    # sql = "select 股东.姓名, 企业股东.任职职位, 企业股东.拥有股份 from 股东 join 企业股东 on 企业股东.股东id = 股东.词条id where 股东.最高学历 = '硕士' order by 企业股东.拥有股份 desc"
    # print(get_sql(schema, sql))
    # sql = "select 企业股东.任职职位 from 股东 join 企业股东 on 企业股东.股东id = 股东.词条id where 股东.最高学历 = '硕士'"
    # print(get_sql(schema, sql))
    # sql = "select 企业.企业名称, 企业.当前股价（美元） from 企业 join 企业股东 on 企业股东.企业id = 企业.词条id where 企业股东.任职职位 = '董事长' group by 企业.词条id order by avg(企业股东.拥有股份) asc limit 3"
    # print(get_sql(schema, sql))
    # sql = "select 股东.姓名 from 股东 join 企业股东 on 股东.词条id = 企业股东.股东id where 企业股东.任职职位 = '董事长'"
    # print(get_sql(schema, sql))
    # sql = "select 股东.姓名, 企业股东.任职职位 from 股东 join 企业股东 on 股东.词条id = 企业股东.股东id where 股东.最高学历 = '硕士' order by 企业股东.拥有股份 desc limit 1"
    # print(get_sql(schema, sql))
    # sql = "select 股东.姓名 from 股东 join 企业股东 on 企业股东.股东id = 股东.词条id where 企业股东.任职职位 = '董事长'"
    # print(get_sql(schema, sql))
    # sql = "select 股东.姓名 from 股东 join 企业股东 on 企业股东.股东id = 股东.词条id where 企业股东.任职职位='董事长' order by 企业股东.拥有股份 desc"
    # sql = "select 股东.姓名 from 股东 join 企业股东 on 企业股东.股东id = 股东.词条id where 企业股东.任职职位='董事长'"
    # print(get_sql(schema, sql))
