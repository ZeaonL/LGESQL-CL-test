...
import six
import sys
from utils_file import evaluate_NL2SQL
from utils_file import is_float
...
def tokenize(string, single_equal=False, math=True):
    ...
    def fn_split1(token):
        result = re.split(sep1, token)
        if "/" in result and len(result) == 3 and result.index("/") == 1:
            i = result.index("/")
            if "（" in result[i - 1] and "）" not in result[i - 1]:
                result = [result[0] + result[1] + result[2]]
        return result

    def _resplit(tmp_tokens, fn_split, fn_omit, optional=False):
        """resplit"""
        new_tokens = []
        for token in tmp_tokens:
            ...
            if fn_omit(token):
                ...
                if optional:
                    new_tokens.extend(fn_split1(token))
                else:
    ...
    tokens_tmp = _resplit(tokens_tmp, lambda x: re.split(sep1, x),
                          lambda x: x in two_bytes_op or x.startswith('"'), optional=True)
...
def parse_condition(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    while idx < len_:
        ...
        if op_str == 'not':
            ...
            op_str = 'not_in'
            idx += 1
        assert idx < len_ and op_str in COND_OPS, "Error condition: idx: {}, tok: {},, toks: {}".format(idx, op_str,
                                                                                                        toks)
...
class Evaluator(object):
    ...
    def eval_partial_match(self, pred, gold, value_match=True):
        ...
        gold_total, pred_total, cnt, cnt_wo_agg = eval_sel(pred,
                                                           gold)  # gold_total：gold里总共select了多少列内容；pred_total：pred里总共select了多少列内容；cnt：select正确的有多少；cnt_wo_agg：不考虑agg后select正确的有多少。
        ...
        gold_total, pred_total, cnt, cnt_wo_agg = eval_where(pred, gold,
                                                             value_match=value_match)  # gold_total:gold里where条件对了多少个, pred_total:pred里where条件对了多少个,cnt：where条件对了多少个；cnt_wo_agg：不考虑op和agg后where正确的有多少。
        acc, rec, f1 = get_scores(cnt, pred_total, gold_total)
        res['where'] = {'acc': acc, 'rec': rec, 'f1': f1, 'gold_total': gold_total, 'pred_total': pred_total}
        acc, rec, f1 = get_scores(cnt_wo_agg, pred_total, gold_total)  # 其实这个计算的不止是no OP，还有no agg
        ...
        gold_total, pred_total, cnt = eval_and_or(pred,
                                                  gold)  # and和or的评估不考虑value的值，只考虑两边的列名表名是否一致。且一个query即使有多个and，也只算一次。
        acc, rec, f1 = get_scores(cnt, pred_total, gold_total)
...
def get_scores(count, pred_total, gold_total):
    ...
    Returns:
    """
    if pred_total != gold_total:  # 如果select的列数pred和gold不一样，则直接返回0
        return 0, 0, 0
    elif count == pred_total:  # 如果一样，则返回1
        return 1, 1, 1
    return 0, 0, 0
...
def count_component1(sql):
    ...
    count += len([token for token in ao if token == "or"])  # or
    cond_units = sql["from"]["conds"][::2] + sql["where"][::2] + sql["having"][::2]
    count += len(
        [
            cond_unit for cond_unit in cond_units if
            (cond_unit[1] == COND_OPS.index('like') or cond_unit[1] == COND_OPS.index('not_like'))
            # like
        ]
    )
...
def eval_sel(pred, gold):
    ...
    for unit in pred_sel:
        if unit in gold_sel:  # 有agg的结果
            cnt += 1
            gold_sel.remove(unit)
        if unit[1] in gold_wo_agg:  # 没有agg的结果
            cnt_wo_agg += 1
...
def eval_having(pred, gold, value_match=True):
    ...
    Returns:
    """
    # modify by shhuang,改成和where子句类似了，如果having里面有多个conds，则每个conds都要比较一次，一次完全一样，则cnt+1
    ...
    #        return [len(gold_conds),len(pred_conds),0]
    # end
    ...
    gold_total = len(gold_conds)
    cnt = 0
    for pred_cond, gold_cond in zip(pred_conds, gold_conds):
...
def eval_and_or(pred, gold):
    # 评价and或者or的时候，是一整个一起评价的，一个query里面，如果有多个and，只要有一个and错误，那整体就算错误，并且一个query里面，total就是1
    """
    Args:
...
def get_keywords(sql):
    # keywords里面少了一个and，如果or是关键词，那and很明显也是关键词呀。然后考虑not的时候，没有考虑not like；考虑in和like的时候没有考虑not in，not like。keyswords里没有from和select这两个关键词。
    """
    Args:
    ...
    if len([token for token in ao if token == 'or']) > 0:
        res.add('or')
    # added by shhuang
    elif len([token for token in ao if token == 'and']) > 0:
        res.add('and')
    ...
    # not keyword
    # modify by shhuang
    if len([cond_unit for cond_unit in cond_units if (cond_unit[1] == 0 or cond_unit[1] == 10)]) > 0:
        res.add('not')

    # in keyword
    if len([cond_unit for cond_unit in cond_units if
            (cond_unit[1] == COND_OPS.index('in') or cond_unit[1] == COND_OPS.index('not_in'))]) > 0:
        res.add('in')

    # like keyword
    if len([cond_unit for cond_unit in cond_units if
            (cond_unit[1] == COND_OPS.index('like') or cond_unit[1] == COND_OPS.index('not_like'))]) > 0:
        res.add('like')
...
def evaluate_complex(table, gold, predict, mode='exact', single_equal=False):
    ...
    all_gold = [(item['query'], item['db_id']) for item in gold]

    with open(predict) as ifs:
        all_pred = [l.strip() for l in ifs if len(l.strip()) > 0]
    ...
    score_hardness = {
        'all': {'count': 0, 'exact': 0, 'acc': 0},
        'easy': {'count': 0, 'exact': 0, 'acc': 0},
        'medium': {'count': 0, 'exact': 0, 'acc': 0},
        'hard': {'count': 0, 'exact': 0, 'acc': 0},
        'extra': {'count': 0, 'exact': 0, 'acc': 0},
    }

    score_hardness_novalue = {
        'all': {'count': 0, 'exact': 0, 'acc': 0},
        'easy': {'count': 0, 'exact': 0, 'acc': 0},
        'medium': {'count': 0, 'exact': 0, 'acc': 0},
        'hard': {'count': 0, 'exact': 0, 'acc': 0},
    ...
    eval_err_num = 0
    for index in range(len(all_gold)):
        scores['all']['count'] += 1
        scores_novalue['all']['count'] += 1
        score_hardness['all']['count'] += 1
        score_hardness_novalue['all']['count'] += 1
        pred_str = all_pred[index]
        gold_str, db_id = all_gold[index]
        ...
        if exact_score == 0:
            logging.debug("error instance %s:\npred: %s\ngold: %s" % (index, pred_str, gold_str))
        scores['all']['exact'] += exact_score
        scores_novalue['all']['exact'] += exact_score_novalue
        # added by shhuang
        score_hardness["all"]['exact'] += exact_score
        score_hardness_novalue['all']['exact'] += exact_score_novalue
        hardness = evaluator.eval_hardness(gold_sql)
        score_hardness[hardness]["count"] += 1
        score_hardness_novalue[hardness]["count"] += 1
        score_hardness[hardness]['exact'] += exact_score
        score_hardness_novalue[hardness]["exact"] += exact_score_novalue
        # 获取预测结果方便进行比对。
        # if exact_score_novalue == 0:
        #     print("%s pred：%s" % (hardness, pred_str))
        #     print("%s gold：%s" % (hardness, gold_str))
        #     print("exact：%d" % (exact_score_novalue))
        #     assert False
        #    print()
    ...
    # added by shhuang
    for hard in score_hardness.keys():
        score_hardness[hard]['acc'] = score_hardness[hard]['exact'] / score_hardness[hard]['count']
    for hard in score_hardness_novalue.keys():
        score_hardness_novalue[hard]['acc'] = score_hardness_novalue[hard]['exact'] / score_hardness_novalue[hard][
            'count']

    return scores, scores_novalue, score_hardness, score_hardness_novalue
...
def evaluate(table, gold, predict, mode='exact', dataset='DuSQL'):
    ...
    """
    if dataset == 'NL2SQL':
        scores, scores_novalue, score_hardness, score_hardness_novalue = evaluate_NL2SQL(table, gold, predict,
                                                                                         mode=mode, single_equal=True)
    elif dataset == 'DuSQL':
        scores, scores_novalue, score_hardness, score_hardness_novalue = evaluate_complex(table, gold, predict,
                                                                                          mode=mode, single_equal=True)
    else:
        scores, scores_novalue, score_hardness, score_hardness_novalue = evaluate_complex(table, gold, predict,
                                                                                          mode=mode, single_equal=True)
...
if __name__ == "__main__":
    ...
    out, out_novalue, out_hardness, out_hardness_novalue = evaluate(args.table, args.gold, args.pred,
                                                                    dataset=args.dataset)
    print('with value:')
    # print(out['all'])
    for kk in out_hardness.keys():
        print(kk, out_hardness[kk])
    print('*' * 20)
    print("without_value")
    for kk in out_hardness_novalue.keys():
        print(kk, out_hardness_novalue[kk])
